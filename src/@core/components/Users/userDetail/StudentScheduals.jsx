...
const StudentScheduals = ({ studentId }) => {
  ...
  const { data: dataToRender } = useQueryGet(
    ["StudentScheduals", studentId],
    `/Schedual/GetStudentScheduals?startDate=1900/01/01&endDate=3000/01/01&StudentId=${studentId}`
  );
  ...
  const { mutate: changeLockToRaise, isSuccess: changeLockToRaiseSucc } =
    useMutationPut("/Schedual/LockToRiase", ["StudentScheduals", studentId]);
...
export default StudentScheduals;
