...
const CourseMentor = ({course}) => {
  ...
  const {data : list  , refetch} = useQueryGet(["courseAss"] , "/CourseAssistance")

  const  courseMentor = list?.filter((item) => item?.courseId === course?.courseId)

  const endOffset = itemOffset + rowsPerPage;
  const currentItems = courseMentor?.slice(itemOffset, endOffset);
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = courseMentor?.filter((reserve) => {
        const startsWith = reserve.assistanceName
          .toLowerCase()
          .startsWith(value.toLowerCase());

        const includes = reserve.assistanceName
          .toLowerCase()
  ...
  const handlePagination = (event) => {
    ...
    const newOffset = (event.selected * rowsPerPage) % courseMentor?.length;
      ...
      <ReactPaginate
        ...
        pageCount={
          searchValue.length
            ...
            : Math.ceil(courseMentor.length / rowsPerPage) || 1
        }
          ...
          <>
            <Row className="justify-content-end align-items-center mx-0 course-reserve-filters">
              ...
              >
                ...
                />
              <Button  color='primary' size='sm' style={{marginRight: "10px" , marginBottom: "5px"}}  onClick={(e) => {
               e.preventDefault();
               toggleEditModal()
                 }}>
                  افزودن منتور 
            ...
            <DataTable
              ...
              columns={CourseMentorColumns(refetch)}
