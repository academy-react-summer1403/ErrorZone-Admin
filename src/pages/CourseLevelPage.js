...
import { CourseLevelColumns } from "../@core/components/courseLevelPages/CourseLevelColumns";
import CreateCourseLevel from "../@core/components/courseLevelPages/CreateCourseLevel";
...
const CourseLevelPage = () => {
  ...
  const { data: list, refetch } = useQueryGet(
    ["courselevel"],
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = list?.filter((reserve) => {
        const startsWith = reserve.levelName
          .toLowerCase()
          .startsWith(value.toLowerCase());

        const includes = reserve.levelName
          .toLowerCase()
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader tag="h4"> سطح بندی دوره ها </CardHeader>
        <div className="react-dataTable user-view-account-projects">
          {list?.length === 0 ? (
            <span className="no-user-course-reserve-found-text">
              متاسفانه سطحی پیدا نشد
            </span>
          ) : (
            <>
              ...
              <DataTable
                ...
                columns={CourseLevelColumns(refetch)}
